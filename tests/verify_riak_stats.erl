...
confirm() ->
    ...
    C = rt_kv:httpc(Node1),
    [rt_kv:httpc_write(C, <<"systest">>, <<X>>, <<"12345">>) || X <- lists:seq(1, 5)],
    [rt_kv:httpc_read(C, <<"systest">>, <<X>>) || X <- lists:seq(1, 5)],
    ...
    rt_kv:pbc_write(Pid, <<"testbucket">>, <<"1">>, <<"blah!">>),
    rt_kv:pbc_set_bucket_prop(Pid, <<"testbucket">>, [{n_val, 4}]),
    
    Stats4 = get_stats(Node1),
    verify_inc(Stats3, Stats4, [{<<"read_repairs_total">>, 0},
                                {<<"read_repairs">>, 0}]),
    
    _Value = rt_kv:pbc_read(Pid, <<"testbucket">>, <<"1">>),
...
get_stats(Node) ->
    timer:sleep(10000),
    StatString = os:cmd(io_lib:format("curl -s -S ~s/stats", [rt_kv:http_url(Node)])),
