...
confirm() ->
    ...
    ?assertMatch({error, {ok, "201", _, _}},
                 rt_kv:httpc_write(HTTP, <<"bz1244bucket">>, undefined, <<"value">>)),

    lager:info("Checking that postcommit fires."),
    ?assertMatch(ok, rt_kv:httpc_write(HTTP, <<"postcommit">>, <<"key">>, <<"value">>)),
