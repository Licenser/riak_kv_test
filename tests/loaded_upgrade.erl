...
seed_cluster(Nodes=[Node1|_]) ->
    ...
    rt_kv:systest_write(Node1, 100, 3),
    ?assertEqual([], rt_kv:systest_read(Node1, 100, 1)),
    ...
    %% For MC Serch
    rt_kv:enable_search_hook(Node1, bucket(search)),
...
seed_search(Pid, [File|Files]) ->
    ...
    rt_kv:pbc_put_file(Pid, bucket(search), Key, File),
    seed_search(Pid, Files).
