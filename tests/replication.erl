...
wait_for_reads(Node, Start, End, Bucket, R) ->
    rt:wait_until(Node,
        fun(_) ->
                rt_kv:systest_read(Node, Start, End, Bucket, R) == []
        end),
    Reads = rt_kv:systest_read(Node, Start, End, Bucket, R),
    lager:info("Reads: ~p", [Reads]),
    length(Reads).

do_write(Node, Start, End, Bucket, W) ->
    case rt_kv:systest_write(Node, Start, End, Bucket, W) of
        [] ->
            [];
        Errors ->
            ...
            timer:sleep(1000),
            lists:flatten([rt_kv:systest_write(Node, S, S, Bucket, W) ||
                    {S, _Error} <- Errors])
    end.
